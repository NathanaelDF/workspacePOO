...
public class EntradaSaida {
	...
	public static int solicitaOpcao() {// mstra uma lista, das ações possiveis dentro do código
	...
	public static String solicitaDescricao(String descricao, int ordem) {// solicita a descricao da casa ou das portas
																			// ou janelas
		if (ordem == 0) {
			return JOptionPane.showInputDialog("Imforma a descrição da " + descricao);
		} else {
	...
	public static int solicitaAberturaMover(ArrayList<Aberturas> listaDeAberturas) {
		...
		for (int i = 0; i < qtdeAbertura; i++) {
			descricoesAberturas[i] = listaDeAberturas.get(i).getDescricao();//colocamos para dentro da variavel descricaoAberturas, todas as descriçoes das aberturas
		}

		String msg = "Escolha a " + tipoAbertura + " a se movimentar";// String para mostra ao usuario qual abertura será movimentada
		...
		if (confirmacao == 0) {
			return exibicaoAberturas.getSelectedIndex();// retrona qual abertura será realizada a mudança
		} else {
			return -1;
		}
	}
	...
	public static void exibeInfoCasa(String informacoes) {
		JOptionPane.showMessageDialog(null, informacoes, "Inforamações da casa", JOptionPane.INFORMATION_MESSAGE);
	}
